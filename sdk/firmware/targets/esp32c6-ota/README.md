...
On-device over-the-air updates for the `smart_thermostat`, delivered by the Chipwright cloud's
...
idf.py -C sdk/firmware/targets/esp32c6-ota menuconfig   # Chipwright OTA menu
```

Set `CW_WIFI_SSID`, `CW_WIFI_PASSWORD`, and `CW_OTA_CLOUD_URL` (e.g.
...
pnpm --filter @chipwright/cloud firmware pubkey-c ~/.chipwright/firmware-signing.key \
...
1. Start the cloud with the matching public key so it verifies published builds:
   ...
   ```sh
   export CHIPWRIGHT_SIGNING_KEY="$(...public key PEM...)"
   PORT=8091 pnpm --filter @chipwright/cloud serve
   ```
...
3. Build a newer image and publish it (the version is embedded in the image, so build it into
   ...
   pnpm --filter @chipwright/cloud firmware publish ~/.chipwright/firmware-signing.key \
     smart_thermostat 1.0.1 sdk/firmware/targets/esp32c6-ota/build-101/chipwright_thermostat_ota.bin \
     http://<host-ip>:8091
   ```
...
4. To see safe rollback, build a version whose self-test fails and publish it as a newer
   ...
   idf.py -C sdk/firmware/targets/esp32c6-ota -B build-102 -DPROJECT_VER=1.0.2 \
     -DCONFIG_CW_OTA_SELFTEST_FAIL=y build
...
- Uses HTTP on the local network for the demo. Production delivery should use HTTPS, which
  the cloud serves when `CHIPWRIGHT_TLS_CERT` and `CHIPWRIGHT_TLS_KEY` are set (see
