...
//
// The reference firmware is telemetry-only, so set_mode is an acknowledged no-op for now; a
// serial command channel in the firmware is what will make actuator control a real operation.
