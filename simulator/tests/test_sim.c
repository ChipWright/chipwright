...
static void test_source_advances(void) {
  ...
  cw_sim_source_init(&source, 20.0f, 0.5f);
  const cw_sensor_driver_t driver = cw_sim_source_driver(&source);
  float value = 0.0f;
...
static void test_fault_none_passthrough(void) {
  cw_sim_source_t source;
  cw_sim_source_init(&source, 10.0f, 1.0f);
  cw_fault_sensor_t sensor;
  cw_fault_sensor_init(&sensor, cw_sim_source_driver(&source));
  const cw_sensor_driver_t driver = cw_fault_sensor_driver(&sensor);
  float value = 0.0f;
  CHECK(driver.read(driver.ctx, &value) == CW_OK);
  CHECK(value == 11.0f);
}
...
static void test_fault_offset(void) {
  cw_sim_source_t source;
  cw_sim_source_init(&source, 0.0f, 0.0f);
  cw_fault_sensor_t sensor;
  cw_fault_sensor_init(&sensor, cw_sim_source_driver(&source));
  const cw_fault_config_t fault = {.kind = CW_FAULT_OFFSET, .offset = 5.0f};
  cw_fault_sensor_set(&sensor, fault);
  const cw_sensor_driver_t driver = cw_fault_sensor_driver(&sensor);
  float value = 0.0f;
  CHECK(driver.read(driver.ctx, &value) == CW_OK);
  CHECK(value == 5.0f);
...
static void test_fault_stuck_freezes(void) {
  cw_sim_source_t source;
  cw_sim_source_init(&source, 0.0f, 1.0f);
  cw_fault_sensor_t sensor;
  cw_fault_sensor_init(&sensor, cw_sim_source_driver(&source));
  const cw_sensor_driver_t driver = cw_fault_sensor_driver(&sensor);

  float first = 0.0f;
  CHECK(driver.read(driver.ctx, &first) == CW_OK);
  CHECK(first == 1.0f);

  const cw_fault_config_t stuck = {.kind = CW_FAULT_STUCK, .offset = 0.0f};
  cw_fault_sensor_set(&sensor, stuck);
  float frozen = 0.0f;
  CHECK(driver.read(driver.ctx, &frozen) == CW_OK);
  CHECK(frozen == first);
  CHECK(driver.read(driver.ctx, &frozen) == CW_OK);
  CHECK(frozen == first);
}
...
static void test_fault_fail_reports_io(void) {
  ...
  const cw_sensor_driver_t driver = cw_fault_sensor_driver(&sensor);
  float value = 0.0f;
  CHECK(driver.read(driver.ctx, &value) == CW_ERR_IO);
}

static cw_sim_source_t g_twin_source;
static cw_fault_sensor_t g_twin_sensor;

static void register_twin_sensor(void) {
  cw_hal_reset();
  cw_sim_source_init(&g_twin_source, 21.0f, 0.5f);
  cw_fault_sensor_init(&g_twin_sensor, cw_sim_source_driver(&g_twin_source));
  cw_hal_register_sensor("temperature_sensor", "celsius", cw_fault_sensor_driver(&g_twin_sensor));
}

static void test_twin_captures_telemetry(void) {
  register_twin_sensor();
  const cw_device_t device = {.name = "twin"};
  cw_twin_capture_t capture;
  cw_twin_capture_reset(&capture);

  CHECK(cw_twin_run(&device, 3, &capture) == CW_OK);
...
static void test_twin_skips_failed_reads(void) {
  register_twin_sensor();
  const cw_fault_config_t fail = {.kind = CW_FAULT_FAIL, .offset = 0.0f};
  cw_fault_sensor_set(&g_twin_sensor, fail);
  const cw_device_t device = {.name = "twin"};
  cw_twin_capture_t capture;
  cw_twin_capture_reset(&capture);

  CHECK(cw_twin_run(&device, 4, &capture) == CW_OK);
  CHECK(capture.count == 0);
}

static void test_twin_commission_survives_loss(void) {
  cw_twin_network_t network;
  cw_twin_network_init(&network, 50, 777);
  cw_matter_session_t session;
  CHECK(cw_twin_commission(&network, "twin", 20, &session) == CW_OK);
...
static void test_twin_networked_lossless_uplink(void) {
  register_twin_sensor();
  cw_twin_network_t network;
  cw_twin_network_init(&network, 0, 1);
  const cw_device_t device = {.name = "twin"};
  cw_twin_capture_t capture;
  cw_twin_capture_reset(&capture);

  CHECK(cw_twin_run_networked(&device, 5, &capture, &network) == CW_OK);
...
static void test_twin_networked_lossy_uplink(void) {
  register_twin_sensor();
  cw_twin_network_t network;
  cw_twin_network_init(&network, 100, 1);
  const cw_device_t device = {.name = "twin"};
  cw_twin_capture_t capture;
  cw_twin_capture_reset(&capture);
...
static cw_status_t command_actuator_set(void *ctx, int mode) {
  (void)ctx;
  g_command_mode = mode;
  return CW_OK;
}
...
static void test_command_apply(void) {
  cw_hal_reset();
  const cw_actuator_driver_t hvac = {.set_mode = command_actuator_set, .ctx = NULL};
  cw_hal_register_actuator("hvac", hvac);

  CHECK(cw_command_apply("command key=hvac mode=1") == CW_OK);
  CHECK(g_command_mode == 1);
  CHECK(cw_command_apply("command key=hvac mode=0") == CW_OK);
  CHECK(g_command_mode == 0);

  // An unknown actuator reports not-found; the fake driver is left untouched.
  CHECK(cw_command_apply("command key=no_such_actuator mode=1") == CW_ERR_NOT_FOUND);
  CHECK(g_command_mode == 0);

  // Malformed and null lines are rejected without touching the actuator.
  CHECK(cw_command_apply("telemetry metric=temperature_sensor value=21") == CW_ERR_INVALID);
  CHECK(cw_command_apply("garbage") == CW_ERR_INVALID);
  CHECK(cw_command_apply(NULL) == CW_ERR_INVALID);
  CHECK(g_command_mode == 0);
}
